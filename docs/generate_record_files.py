...
async def generate_file_for_specific_record(record_uid_str: str, file_size_kb: int = 15):
    ...
    try:
        ...
        if result.get("success"):
            ...
            print(f"❌ Failed to generate file: {result.get('error', 'Unknown error')}")
    ...
    try:
        ...
        for result in results:
            if result.get("success"):
                successful += 1
                upload_info = result.get("upload_result", {})
                print(f"✅ {result['record_uid'][:8]}... -> {upload_info.get('object_key', 'N/A')}")
            else:
                failed += 1
                print(f"❌ {result['record_uid'][:8]}... -> {result.get('error', 'Unknown error')}")
